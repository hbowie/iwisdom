...
* An import tab has been added. Two new XML import formats are now supported: the Quotation Markup Language (QML), and the Quotation Exchange Language (QEL). In addition, xml formats can now be imported from a Web URL, as well as from a local file. PortableWisdom.org, for example, now has <xml> links that can be used for imports.
